...
class WaveNet(object):
    def __init__(self,
        ...
        self.save_path = './bongsang_model.ckpt'
    ...
    def train(self, inputs, targets):
        ...
        while not terminal:
            ...
            if cost < 1e-1:
                terminal = True
            losses.append(cost)
            print('train #{}, loss={}'.format(i, losses[-1]))

            #if i % 50 == 0:
                #plt.plot(losses)
                #plt.show()
        saved_path = self.saver.save(self.sess, self.save_path)
        print('Training #{} is done and saved to {}'.format(i, saved_path))
    ...
    def test(self, inputs, targets):
        ...
        print('Loading saved model from {}'.format(self.save_path))
        cost = self._train(inputs, targets)
        print('Test loss={}'.format(cost))
    ...
    def run(self, input, num_samples):
        ...
        for step in range(num_samples):
            ...
            if step % 1000 == 0:
                ...
                print('Sampling step #{}, signal prediction = {}'.format(step, predictions_[0, -1]))
